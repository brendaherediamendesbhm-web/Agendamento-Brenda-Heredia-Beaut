...
// --- CONFIGURAÇÃO DE IMPORTS DO FIREBASE (SE DISPONÍVEL NO CONTEXTO) ---
...
try {
  ...
  console.log("A rodar em modo local (armazenamento em memória / localStorage).");
}
...
export default function App() {
  ...
  const [firebaseStatus, setFirebaseStatus] = useState(firebaseAvailable ? 'connecting' : 'local');
  const [toastMessage, setToastMessage] = useState(null);
  const [toastType, setToastType] = useState('success'); 

  // Controle de segurança para simular que a cliente não acede ao painel admin
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState(false);
  const [adminPassword, setAdminPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);

  // Lista de serviços oferecidos por Brenda Heredia Beauty (Valores em Reais Reduzidos e Tempos de Iniciante)
  const services = [
    ...
    if (firebaseAvailable && db && auth) {
      const initAuthAndFirestore = async () => {
        try {
          ...
          setFirebaseStatus('fallback-local');
          loadLocalData();
        }
      };
    ...
    if (firebaseAvailable && db && user) {
      ...
      const unsubscribe = onSnapshot(appointmentsRef, 
        ...
        (error) => {
          ...
          showToast("Erro ao sincronizar com a nuvem. Usando dados locais.", "error");
        }
      );
  ...
  const handleCreateAppointment = async (appointmentData) => {
    ...
    if (firebaseAvailable && db && user) {
      try {
        ...
        console.error("Erro ao agendar no Firebase:", err);
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    if (firebaseAvailable && db && user && !id.startsWith('mock-') && !id.startsWith('local-')) {
      try {
        ...
        showToast(`Marcação atualizada para ${newStatus}!`, "success");
      } catch (err) {
        console.error("Erro ao atualizar:", err);
        showToast("Não foi possível atualizar na nuvem.", "error");
      }
    } else {
      ...
      showToast(`Marcação updated para ${newStatus}!`, "success");
    }
  };
  ...
  const [chatMessages, setChatMessages] = useState([
    ...
    { sender: 'bot', text: 'Deseja agendar um serviço, consultar valores ou falar connosco? Selecione uma das opções abaixo:', time: 'Agora', isOptions: true }
  ]);
  ...
  const handleSendChatMessage = (text, isSystemOption = false) => {
    ...
    setTimeout(() => {
      ...
      if (isSystemOption && text === '1. Marcar Novo Serviço') {
        ...
        botResponse.push({ sender: 'bot', text: 'Por favor, clique no botão rosa "ABRIR AGENDA ONLINE" que acabou de aparecer acima do telemóvel para escolher o seu serviço e o melhor horário na minha agenda inteligente!', time: 'Agora', showLinkTrigger: true });
        ...
        botResponse.push({ sender: 'bot', text: 'Estamos localizadas num espaço super acolhedor! 📍 Jardim Progresso, Rua Urias Ribeiro, n⁰ 2551, Casa 21, Condomínio Espanha, próximo ao colégio Objetivo.', time: 'Agora' });
  ...
  return (
    <div className="min-h-screen bg-[#FAF6F0] text-[#4A3F3B] flex flex-col antialiased">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white p-6 rounded-2xl border border-[#F0E6DC] shadow-sm">
                ...
                <p className="text-sm text-[#7D6B63] mt-3 leading-relaxed">
                  Para não perder tempo a responder às mesmas mensagens no WhatsApp, este robô atende a sua cliente instantaneamente 24h por dia. Ele faz o filtro inicial e entrega o link da sua agenda online personalizada.
                </p>
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#7D6B63] mt-0.5">O bot envia um botão exclusivo. Ao clicar, a cliente abre apenas o portal de marcação no ecrã inteiro do telemóvel dela.</p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#7D6B63] mt-0.5">A marcação cai no seu painel privado onde você pode aprovar, cancelar ou enviar o lembrete de confirmação de segurança.</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-[#FAF6F0] p-4 rounded-xl border border-[#EBE0D5] flex gap-3 items-start">
                ...
                <p className="text-xs text-[#7D6B63] leading-relaxed">
                  <strong>Simule o comportamento do bot:</strong> Use o telemóvel ao lado para testar a experiência de uma cliente. Veja como as respostas automáticas resolvem a dúvida em segundos!
                </p>
              </div>
            </div>

            {/* Direita: Mockup do Telemóvel */}
                      ...
                      <div key={index} className={`flex flex-col ${msg.sender === 'client' ? 'items-end' : 'items-start'} animate-fadeIn`}>
                        <div className={`max-w-[85%] rounded-2xl p-3 text-xs leading-relaxed ${
                          ...
                          {msg.showLinkTrigger && (
                            <button 
                              onClick={() => {
                                showToast("A carregar a agenda...", "info");
            ...
            {!isAdminLoggedIn ? (
              <div className="bg-white p-8 rounded-2xl border border-[#F0E6DC] shadow-sm max-w-md mx-auto text-center space-y-6 mt-12">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#9E8B83] mt-2">Esta área contém relatórios financeiros e dados confidenciais das clientes. Introduza a senha profissional para acessar.</p>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-[#F0E6DC] shadow-xs">
                  <div>
                    <h2 className="text-xl font-bold font-serif-elegant text-[#4A3F3B]">Gestão de Marcações - Brenda Heredia</h2>
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setAdminPassword('');
                      showToast("Sessão profissional terminada.", "info");
                    }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-white p-4 rounded-xl border border-[#F0E6DC] flex items-center justify-between shadow-xs">
                    <div>
                      <p className="text-[11px] text-[#9E8B83] font-semibold uppercase tracking-wider">Total de Marcações</p>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-[#F0E6DC] flex items-center justify-between shadow-xs">
                    <div>
                      <p className="text-[11px] text-[#9E8B83] font-semibold uppercase tracking-wider">Faturação Estimada</p>
                ...
                <div className="bg-white rounded-2xl border border-[#F0E6DC] shadow-xs overflow-hidden">
                  ...
                  {appointments.length === 0 ? (
                    <div className="p-12 text-center">
                      ...
                      <h5 className="text-sm font-bold text-[#4A3F3B] mt-4">Nenhuma marcação na sua lista</h5>
                    </div>
                  ) : (
                    <div className="divide-y divide-[#F0E6DC]">
                      {appointments.map((appointment) => {
                        ...
                        const formattedDate = !isNaN(parsedDate) 
                          ? parsedDate.toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' })
                          : 'Data inválida';
      ...
      {/* --- RODAPÉ INSTITUCIONAL EXCLUSIVO --- */}
...
function BookingWizard({ services, appointments, onComplete }) {
  ...
  // Estados de navegação do calendário mensal completo
  const todayDate = new Date(2026, 4, 27); // Data fixa do dia atual: Maio 27, 2026
  const [viewDate, setViewDate] = useState(new Date(2026, 4, 1)); // Iniciando visualização em Maio 2026

  const categories = ['Novos & Blindagem', 'Manutenções', 'Remoções e Extras'];

  // Função que monta a estrutura de grade do calendário mensal completo
  ...
  // Verifica se uma determinada data do calendário mensal está bloqueada ou indisponível
  const isDateDisabled = (dayNum) => {
    ...
    // Bloqueia sábados (6) e domingos (0)
    if (dayOfWeek === 0 || dayOfWeek === 6) return true;

    // Bloqueia dias anteriores à data atual (Maio 27, 2026)
    const minDate = new Date(2026, 4, 27);
    if (checkDate < minDate) return true;

    // Limita o agendamento a um mês completo de antecedência (máximo de 30 dias a partir de hoje)
  ...
  // Retorna os horários dinamicamente conforme o dia da semana e evita colisões e sobreposições de horários com atendimentos já feitos
  const getAvailableTimeSlots = (dateStr) => {
    if (!dateStr || !selectedService) return [];
    
    // Parsing manual da string de data para contornar problemas de timezone do navegador
    const [year, month, dayNum] = dateStr.split('-').map(Number);
    const date = new Date(year, month - 1, dayNum);
    const day = date.getDay(); // 1: Seg, 2: Ter, 3: Qua, 4: Qui, 5: Sex
    
    let baseSlots = [];
    if (day === 5) { // Sexta-feira: 08:00 até às 15:00
      baseSlots = ['08:00', '09:30', '11:00', '12:30', '14:00', '15:00'];
    } else { // Segunda a Quinta: 10:00 até às 17:00
      baseSlots = ['10:00', '11:30', '13:00', '14:30', '16:00', '17:00'];
    }

    // Auxiliar para converter formato "HH:MM" em minutos totais a partir de 00:00
    const timeToMins = (t) => {
      if (!t) return 0;
    ...
    // Auxiliar para converter formato de duração "XhYY" em minutos totais
    const durToMins = (d) => {
      if (!d) return 0;
    ...
    // Duração do procedimento atual que o usuário quer marcar
    ...
    return baseSlots.filter((slotTime) => {
      ...
      // Verifica se o slot pretendido se sobrepõe a QUALQUER marcação já existente no mesmo dia
      const isOverlapping = appointments && appointments.some((app) => {
        ...
        // Busca o serviço existente correspondente para ver a sua duração
        const existingService = services.find((s) => s.id === app.serviceId || s.name === app.serviceName);
        const existingDuration = existingService ? durToMins(existingService.duration) : 120; // fallback para 2h se não achar
        const appEnd = appStart + existingDuration;

        // Há colisão se: o início do slot for antes do fim do atendimento atual, E o fim do slot for depois do início do atendimento atual
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#F0E6DC] shadow-sm overflow-hidden">
      ...
      <div className="p-6 md:p-8">
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedTime(''); // Reseta horário ao mudar a data
                          }}
        ...
        {/* ================= PASSO 3: DADOS DE CONTACTO E CONFIRMAÇÃO ================= */}
        {step === 3 && (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="text-center max-w-md mx-auto">
              <h3 className="text-xl font-bold font-serif-elegant text-[#4A3F3B]">Só precisamos do seu contacto!</h3>
              <p className="text-xs text-[#9E8B83] mt-1">Preencha o seu nome e telemóvel para podermos enviar o lembrete de confirmação de segurança.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#9E8B83] uppercase tracking-wider flex items-center gap-1">
                  <Smartphone className="w-3.5 h-3.5 text-[#B57C74]" /> Telemóvel (WhatsApp)
                </label>
                <input 
                  type="tel"
                  required
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <textarea 
                  rows="2"
                  placeholder="Diga se quer alguma cor específica, unha decorada, ou se tem alguma unha partida."
                  value={notes}
            ...
            <div className="bg-[#FAF6F0] p-4 rounded-xl border border-[#EBE0D5] text-xs space-y-2">
              <h4 className="font-bold text-[#4A3F3B] border-b border-[#EBE0D5] pb-1.5 mb-1.5 uppercase tracking-widest text-[9px] text-[#B57C74]">Resumo da Marcação</h4>
          ...
          <div className="py-8 text-center space-y-6">
            ...
            <div className="bg-[#FAF6F0] p-4 rounded-xl border border-[#EBE0D5] text-xs max-sm mx-auto space-y-2">
              <p className="text-[#7D6B63]">
                ...
                <br />
                O robô registou a sua marcação na agenda. A Brenda confirmará diretamente consigo por mensagem de WhatsApp.
              </p>
            </div>
